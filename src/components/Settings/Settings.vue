  ...
  <v-card v-if="!isLoading" flat tile v-scroll="onPageScroll" class="mx-0 pb-4">
    <v-card-title class="titleTabs  pa-0" :style="titleTabsStyle">
      <v-tabs v-model="tabs" @change="onTabChange" class="text-center px-0">
        ...
        <v-tab href="#board">
          <v-icon left> mdi-tablet-dashboard </v-icon> BOARD
        </v-tab>
...
export default {
  ...
  computed: {
    titleTabsStyle() {
      ...
      const isBulletinboard =
        this.$route.name === 'Bulletinboard' ||
        headerSettings.boardHeaderPreview
      let top =
        isBulletinboard && headerSettings?.boardHeaderShow
          ? +headerSettings.boardHeaderHeight
          : +appSettings.appHeaderHeight
      const style = {
        top: `${top}px`,
      }
      return style
