<template>
  ...
  >
    ...
    <v-icon class="flex-shrink-1 px-4" :color="settings.tickerTextColor"
...
export default {
  ...
  methods: {
    async feedFetch(rss) {
      ...
      if (!rss)
        return [
          'Welcome to Vuelletinboard!!',
          'Welcome to Vuelletinboard!!',
          'Welcome to Vulletinboard!!',
          'Welcome to Vuelletinboard!!',
          'Welcome to Vulletinboard!!',
        ]
    ...
    timeUpdate() {
      ...
      if (h == 0) h = 12
      this.date = `${M.toString().padStart(2, '0')}/${D.toString().padStart(
        2,
        '0',
      )}/${Y}`
  ...
  async created() {
    ...
    if (this?.settings?.tickerRss === 'custom') {
      ...
      try {
        ...
        console.log(
          `error fetching configured feed: ${this.settings.tickerRss}`,
        )
      }
    }
