...
export default {
  ...
  methods: {
    async init() {
      ...
      //IF ITEMS ,SETTINGS
      //TODO: REFACTOR THIS WORKAROUND HACK
      const itemsDisplayed = this.items.filter((i) => i.visible == true)
      this.$store.dispatch('itemsSet', itemsDisplayed)
      this.resetInterval(this.itemNext, this.settings.boardItemTimeout)
    ...
    setHeight() {
      ...
      const headerHeight = !headerShow
        ? 0
        : this?.header?.boardHeaderHeight || defaults.header
      const tickerHeight =
        this?.ticker?.tickerShow == false
          ? 0
          : this?.ticker?.tickerHeight || defaults.ticker
      const adj = 0
...
<template>
  <v-sheet
    :color="settings.boardBackground"
    height="100%"
    ref="contentSheet"
    width="100%"
    class="contentSheet d-flex flex-no-wrap align-start justify-start"
  >
    <!--EVENT LIST -->
    <EventList
      v-if="!isLoading"
      @showNext="itemNext"
      @showPrevious="itemPrevious"
